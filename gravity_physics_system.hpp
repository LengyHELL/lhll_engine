...
class GravityPhysicsSystem {
 public:
  ...
  void update(std::vector<LhllGameObject>& objs, float dt, unsigned int substeps = 1) {
  ...
  glm::vec2 computeForce(LhllGameObject& fromObj, LhllGameObject& toObj) const {
 ...
 private:
  void stepSimulation(std::vector<LhllGameObject>& physicsObjs, float dt) {
...
class Vec2FieldSystem {
 public:
  void update(
      ...
      std::vector<LhllGameObject>& physicsObjs,
      std::vector<LhllGameObject>& vectorField) {
...
std::unique_ptr<LhllModel> createSquareModel(LhllDevice& device, glm::vec2 offset) {
...
std::unique_ptr<LhllModel> createCircleModel(LhllDevice& device, unsigned int numSides) {
  std::vector<LhllModel::Vertex> uniqueVertices{};
...
void FirstApp::run() {
  // create some models
  std::shared_ptr<LhllModel> squareModel = createSquareModel(
      lhllDevice,
      {.5f, .0f});  // offset model by .5 so rotation occurs at edge rather than center of square
  std::shared_ptr<LhllModel> circleModel = createCircleModel(lhllDevice, 64);

  // create physics objects
  std::vector<LhllGameObject> physicsObjects{};
  auto red = LhllGameObject::createGameObject();
  ...
  auto blue = LhllGameObject::createGameObject();
  ...
  for (int i = 0; i < gridCount; i++) {
    for (int j = 0; j < gridCount; j++) {
      auto vf = LhllGameObject::createGameObject();
